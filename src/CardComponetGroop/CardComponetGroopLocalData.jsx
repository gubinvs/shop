...
const CardComponetGroopLocalData = ({ h2, nomenclature }) => {
  ...
  // ✅ Обновляем товары, когда приходит новая номенклатура
  useEffect(() => {
    if (Array.isArray(nomenclature) && nomenclature.length > 0) {
      setItems(nomenclature);
      setQuantities(nomenclature.map(() => 0)); // для контроля количества
      setLoading(false);
    }
  }, [nomenclature]);
                ...
                <div className="card-component__basket-block">
                  <div className="basket-block__quantity-item">
                    <div
                      ...
                      onClick={() => handleDecrement(globalIndex)}
                    >
                      −
                    </div>
                    ...
                    <div
                      ...
                      onClick={() => handleIncrement(globalIndex)}
                    >
                      +
                    </div>
                  </div>
                  <button
                    className={`basket-block__button ${
                      isInBasket(globalIndex) ? "added" : ""
                    } ${quantities[globalIndex] === 0 ? "disabled" : ""}`}
